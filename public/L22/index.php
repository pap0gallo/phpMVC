...
$repo = new App\L22\src\PostRepository();
...
$app->post('/posts', function ($request, $response) use ($repo, $router) {
    ...
    $validator = new App\L22\src\Validator();
...
$app->delete('/posts/{id}', function ($request, $response, $args) use ($container, $repo, $router) {
    $id = (string) $args['id'];
    $postData = $repo->find($id);
    if (!empty($postData)) {
        $repo->destroy($id);
        $container->get('flash')->addMessage('success', 'Post has been removed');
